...
namespace Azure.AI.FormRecognizer.Models
{
    ...
    public class RecognizeContentOperation : Operation<FormPageCollection>
    {
        ...
        private async Task<Response<AnalyzeOperationResult>> GetResponseAsync(CancellationToken cancellationToken) =>
            await _serviceClient.GetAnalyzeLayoutResultAsync(new Guid(Id), cancellationToken).ConfigureAwait(false);

        private Response<AnalyzeOperationResult> GetResponse(CancellationToken cancellationToken) =>
            _serviceClient.GetAnalyzeLayoutResult(new Guid(Id), cancellationToken);

        private static OperationInternalResponseStatus GetStatus(Response<AnalyzeOperationResult> response) =>
            response.Value.Status switch
            {
                OperationStatus.Succeeded => OperationInternalResponseStatus.Succeeded,
                OperationStatus.Failed => OperationInternalResponseStatus.Failed,
                _ => OperationInternalResponseStatus.Pending
            };

        private static FormPageCollection ParseResponse(Response<AnalyzeOperationResult> response)
        {
